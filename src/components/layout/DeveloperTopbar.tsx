...
const pageLabels: Array<{ href: string; label: string; exact?: boolean }> = [
  { href: '/developer/api-keys', label: 'إدارة مفاتيح الـ API' },
  { href: '/developer/webhooks', label: 'الويبهوك (Webhooks)' },
  { href: '/developer/docs', label: 'وثائق المطورين' },
  { href: '/developer/sdks', label: 'المكتبات والـ SDKs' },
  { href: '/developer', label: 'لوحة تحكم المطور', exact: true },
];

function getPageLabel(pathname: string): string {
  const matched = pageLabels.find((item) => item.exact ? pathname === item.href : pathname.startsWith(item.href));
  return matched ? matched.label : 'مركز المطورين';
}
    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200 dark:border-[#1A2A4A] bg-white/95 dark:bg-[#050A19]/95 backdrop-blur-md text-slate-900 dark:text-white transition-colors duration-300">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-3 px-3 py-3 md:px-5 md:py-4">
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            aria-label="الملف الشخصي للمطور"
          ...
          >
            ...
            <div className="text-right hidden sm:block">
               <p className="text-sm font-bold text-slate-900 dark:text-white">أحمد المطور</p>
               <p className="text-[10px] text-blue-500">مطور تطبيقات</p>
            </div>
          </button>

          <button className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition" aria-label="إعدادات المطور">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden lg:block">
            <h1 className="text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
                {pageLabel}
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-500/20 text-blue-500 border border-blue-500/30">Dev Mode</span>
            </h1>
        </div>
