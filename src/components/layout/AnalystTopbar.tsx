...
const pageLabels: Array<{ href: string; label: string; exact?: boolean }> = [
  { href: '/analyst/retail', label: 'تحليل مبيعات التجزئة' },
  { href: '/analyst/models', label: 'نماذج الذكاء الاصطناعي' },
  { href: '/analyst/prep', label: 'معاينة وتجهيز البيانات' },
  { href: '/analyst/builder', label: 'بناء اللوحات' },
  { href: '/analyst/ai-builder', label: 'توليد اللوحات بالذكاء الاصطناعي' },
  { href: '/analyst/chat', label: 'التحدث مع البيانات' },
  { href: '/analyst/collab', label: 'التعاون والمشاركة' },
...
function getPageLabel(pathname: string): string {
  ...
  return matched ? matched.label : 'مصادر البيانات';
}
      ...
      <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-3 px-3 py-3 md:px-5 md:py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div className="h-9 w-9 overflow-hidden rounded-full bg-slate-800">
               <img src="/avatars/ahmed.jpg" alt="" className="h-full w-full object-cover" onError={(e) => { e.currentTarget.src = 'https://ui-avatars.com/api/?name=Ahmed&background=1e293b&color=fff' }} />
            </div>
            <div className="text-right hidden sm:block">
               <p className="text-sm font-bold text-slate-900 dark:text-white">أحمد العتيبي</p>
               <p className="text-[10px] text-blue-400">البيانات</p>
            </div>
          </button>

          <button className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition" aria-label="الإعدادات">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden lg:block">
            <h1 className="text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
                {pageLabel}
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">مباشر</span>
            </h1>
        </div>
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto justify-end">
          ...
          >
            ...
            <span className="absolute top-1.5 right-2 h-2 w-2 rounded-full bg-rose-500 border-2 border-white dark:border-[#0A1126]" />
          </button>

          {/* Spacer Line */}
          <div className="h-6 w-px bg-slate-200 dark:bg-[#1A2A4A]" />

          <div className="relative shrink-0 rounded-full border border-slate-200 dark:border-[#1A2A4A] bg-slate-50 dark:bg-[#0A1126] p-1.5 text-slate-500 dark:text-slate-400 transition hover:bg-slate-100 dark:hover:bg-[#142247] hover:text-slate-900 dark:hover:text-white group flex items-center justify-center">
             <ThemeToggle />
          </div>
