          ...
          <section className="flex-1 rounded-3xl border border-[#1A2A4A] bg-[#080F23] p-4 md:p-6">
              ...
              <div className="flex items-center gap-3">
                <div className="relative w-full max-w-xs">
                  <input
                    ...
                    readOnly
                    value="بحث في النظام..."
                    className="w-full rounded-xl border border-[#1A2A4A] bg-[#0D1632] px-10 py-2.5 text-sm text-slate-300 outline-none"
                  />
                ...
                <span className="rounded-full bg-amber-500/15 px-3 py-1 text-xs font-bold text-amber-300">API Latency: High</span>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[320px_1fr]">
              <article className="rounded-2xl border border-[#1A2A4A] bg-[#0D1632] p-4">
                ...
                <div className="mt-5 space-y-5">
                  <div>
                    ...
                    <div className="h-2 rounded-full bg-[#162341]">
                      <div className="h-2 w-[45%] rounded-full bg-blue-400" />
                    </div>
                  </div>
                  <div>
                    <div className="mb-2 flex items-center justify-between text-xs text-slate-400">
                    ...
                    <div className="h-2 rounded-full bg-[#162341]">
                      <div className="h-2 w-[38%] rounded-full bg-violet-400" />
                    </div>
                  </div>
                  <div>
                    <div className="mb-2 flex items-center justify-between text-xs text-slate-400">
                    ...
                    <div className="h-2 rounded-full bg-[#162341]">
                      <div className="h-2 w-[17%] rounded-full bg-cyan-400" />
                    </div>
                  </div>
                </div>

                <div className="mt-6 rounded-xl border border-[#1A2A4A] bg-[#0A132C] p-3 text-xs text-slate-400">
                  إجمالي الإيرادات السنوية
                  <p className="mt-1 text-base font-black text-blue-300">$1.2M</p>
                </div>
              </article>
              ...
              <article className="rounded-2xl border border-[#1A2A4A] bg-[#0D1632] p-4">
                ...
                <div className="relative h-[260px] overflow-hidden rounded-xl border border-[#1A2A4A] bg-[#0A132C] p-4">
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(59,130,246,0.15),transparent_55%)]" />
            ...
            <article className="mt-4 rounded-2xl border border-[#1A2A4A] bg-[#0D1632] p-4">
              <div className="mb-4 flex items-center justify-between">
                ...
                <table className="w-full min-w-[760px] text-right">
                  <thead>
                    <tr className="border-b border-[#1A2A4A] text-xs text-slate-400">
                      ...
                      <th className="pb-3 font-semibold">المستخدم / المصدر</th>
                      <th className="pb-3 font-semibold">العملية</th>
                  ...
                  <tbody>
                    {activityRows.map((row) => (
                      <tr key={`${row.type}-${row.at}`} className="border-b border-[#15203B] text-sm text-slate-300">
                        <td className="py-3">
                          <span className={`rounded-md px-2 py-1 text-[11px] font-bold ${row.badge}`}>{row.type}</span>
                        </td>
                        <td className="py-3">{row.actor}</td>
                        <td className="py-3">{row.action}</td>
                        <td className={`py-3 font-bold ${row.status === 'فشل' ? 'text-rose-400' : row.status === 'قيد التنفيذ' ? 'text-sky-300' : 'text-emerald-400'}`}>
                          {row.status}
                        </td>
